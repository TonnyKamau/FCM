...
# Load .env so secrets never need hardcoded fallbacks. Uses python-dotenv when
# available, otherwise a minimal parser — no extra dependency required.
def _load_dotenv():
    try:
...
# Load .env for local dev; on PythonAnywhere set vars in Web tab → Environment variables.
# If python-dotenv is not installed the import error is silently ignored.
try:
    from dotenv import load_dotenv
...
# Checks FIREBASE_CREDENTIALS_PATH first, then GOOGLE_APPLICATION_CREDENTIALS,
# then falls back to serviceAccountKey.json in the same folder as this file.
FIREBASE_CREDENTIALS_PATH = (
    os.getenv("FIREBASE_CREDENTIALS_PATH")
    or os.getenv("GOOGLE_APPLICATION_CREDENTIALS")
    or os.path.join(os.path.dirname(os.path.abspath(__file__)), "serviceAccountKey.json")
)
...
# API key for the media upload server (api.kit-ifms.com) — the same key the
# Android app uses (local.properties API_KEY). Distinct from this backend's
# own API_KEY above.
MEDIA_UPLOAD_API_KEY = os.getenv("MEDIA_UPLOAD_API_KEY", "")
...
SMTP_SENDER_EMAIL    = os.getenv("SMTP_SENDER_EMAIL",    "kit.erpsolutions@gmail.com")
...
# Safaricom calls this URL with the STK push result.
# Matches STKCALLBACKURL in the Android BuildConfig.
MPESA_STK_CALLBACK_URL = os.getenv(
    "MPESA_STK_CALLBACK_URL",
    "https://kit-ifms.com/node_modules/online/stk_callback.php",
)
